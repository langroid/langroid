...
class PGVector(VectorStore):
    ...
    def delete_collection(self, collection_name: str) -> None:
        if collection_name in _classes: # clear cached embedding table
            ...
            metadata = MetaData()
            metadata.reflect(bind=self.engine) # get the current db state
...
def test_create_collection():
    ...
    assert inspector.has_table("test_collection1"), "Table 'test_collection' does not exist"
    
    postgres_db.delete_collection("test_collection1")
    inspector = inspect(engine)
    assert not inspector.has_table("test_collection1"), "Table 'test_collection' was not deleted"
