...
class Agent(ABC):
    ...
    def agent_response(
        ...
        return ChatDocument(
            content=results if isinstance(results, str) else "",
            oai_tool_id2result=results if isinstance(results, dict) else None,
    ...
    def handle_message(
        ...
        if has_ids and len(id2result) > 1:
            # if there are multiple results, return them as a dict
            # (only applies to OpenAI tool-calls)
            return id2result
