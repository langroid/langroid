...
class AzureConfig(OpenAIGPTConfig):
    ...
    ``.env`` after renaming the file to ``.azure_env``. Because this file
    is used by this class to find the env vars.
    Attributes:
        type (str): should be ``azure``
        api_version (str): can be set inside the ``.env``
        deployment_name (str): can be set inside the ``.env`` and should be based
        the custom name you chose for your deployment when you deployed a model
        model (str): can be set inside the ``.env`` and should be based on
        the model name
    """
