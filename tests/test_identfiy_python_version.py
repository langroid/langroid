...
""",
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        for filename, content in files_content.items():
            ...
            if filename.endswith('.toml') or filename.endswith('.cfg') or filename == "Pipfile":
...
def test_get_python_version_from_pipfile(setup_files):
    assert pyver.get_python_version_from_pipfile(setup_files) == 'According to Pipfile python' + test_version


def test_get_python_version_from_pipfile_lock(setup_files):
    assert pyver.get_python_version_from_pipfile_lock(setup_files) == 'According to Pipfile.lock python' + test_version
