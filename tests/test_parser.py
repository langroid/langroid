...
# pytest parametrize over split = "recursive" and split = "para_sentence"
# pytest parametrize over length_function = char_length and length_function = token_length
@pytest.mark.parametrize("splitter, expected", [
    ("recursive", 3),
    ("para_sentence", 3),
])
def test_parser(splitter:str, expected:int):
    cfg = ParsingConfig(
        splitter=splitter,
        chunk_size=1000,
        chunk_overlap=2,
    ...
    assert len(split_docs) == expected
